...
pub async fn get_shell(env: &Environment) -> core::result::Result<std::process::Child, std::io::Error> {
    ...
    std::env::set_var("NP_RUNTIME", "proot");
    std::env::set_var("NIXPKGS_ALLOW_UNFREE", "1");
    let cmd = format!(r#"HOME=/home/{}/nixHome {} nix-shell /home/marko/envie/Environments/default.nix --verbose --pure --keep HOME --option sandbox false --command "echo 'Entered the Nix env'; export SSL_CERT_FILE=/nix/store/2ymr3vj3sxgcpvwnrfwpz8d2zar030gq-nss-cacert-3.74/etc/ssl/certs/ca-bundle.crt; export HOME=/home/$USER; {}""#, &os_username(), nix_portable_exe().to_str().unwrap(), startup_command);
    println_!(&cmd);
